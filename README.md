...
- `xtquant` / `pytdx` / `akshare` 均为**函数内惰性导入**：缺失时该数据源报错/降级，不影响其它源、不影响模块启动。
...
| **TDX(通达信)** | `pytdx` 包 + 网络可达通达信行情服务器（默认 5 台） | 丢 A股行情/PB 第三源 |
...
| 场景 | 必装 |
| --- | --- |
| 只要行情多源 | Python + `pytdx` + `akshare`（+ 网络）→ TDX/新浪/Yahoo/东财 即可 |
| + A股/港股行情主源 | 装 **miniQMT 终端** + venv 里 `xtquant` |
| + hithink 财务/特色 | 配 **hithink API Key** |
| 一键跑回归 | venv 装 `pytdx akshare pandas`（`test_all.py` 里 `PY` 改本机 venv 路径） |
